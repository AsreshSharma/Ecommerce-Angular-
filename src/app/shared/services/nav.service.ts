...
export class NavService {
	...
	MENUITEMS: Menu[] = [
		...
		{			
			path: '/home/fashion/refer_earn', title: 'Refer & Earn', type: 'link'
		},
		{			
			path: '/pages/contact', title: 'Contact', type: 'link'
		}
	...
	// Product
	private get products(): Observable<Menu[]> {
		...
		this.Menus = this.http.post<any>(apiConfig.leftmenuitems, formData).pipe(map(data => data[0]));
		return this.Menus;
	}
	// Array
	items = new BehaviorSubject<Menu[]>(this.MENUITEMS);
	leftMenuItems =this.products;
